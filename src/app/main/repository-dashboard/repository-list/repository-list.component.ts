...
export class RepositoryListComponent extends AbstractComponent {
  ...
  readonly qts = new QuickTableSelect(this, () => this.repositories)
    .addAction({
  ...
  ngOnInit(): void {
    ...
    this.repositoryModified!
      .pipe(mergeMap(event => {
        ...
        return (repository ? this.loadRepository(repository) : utils.ofVoid());
      }))
  ...
  onFetchAllClicked() {
    multiFork(environment.concurrency, this._repositories.map(repo => this.fetchAndReloadRepository(repo)))
      .pipe(this.untilDestroyed())
      .subscribe(utils.subscriber());
    return false;
  }
  ...
  onAction(repo: RepositoryListItem, action: 'fetch') {
    this.fetchAndReloadRepository(repo).pipe(this.untilDestroyed()).subscribe(utils.subscriber());
  }

  onBulkAction(action: 'fetch') {
    multiFork(environment.concurrency, this.qts.selected.map(repo => this.fetchAndReloadRepository(repo)))
      .pipe(this.untilDestroyed())
      .subscribe(utils.subscriber());
  }
