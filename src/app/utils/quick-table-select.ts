...
export class QuickTableSelect<T> {
  ...
  constructor(private readonly _parent: AbstractComponent, private readonly callback: () => T[]) {
  ...
  untilDestroyed<U>(): MonoTypeOperatorFunction<U> {
   return this._parent.untilDestroyed();
  }
}
...
export class QuickTableAction<T> {
  ...
  invokeSingle(item: T): void {
    this.invoker(item)
      .pipe(this.table.untilDestroyed())
      .subscribe(utils.subscriber());
  }

  invokeAll(): void {
    multiFork(environment.concurrency, this.table.selected.map(item => this.invoker(item)))
      .pipe(this.table.untilDestroyed())
      .subscribe(utils.subscriber());
  }
}
